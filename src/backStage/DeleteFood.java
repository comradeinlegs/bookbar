...
public class DeleteFood extends HttpServlet implements Delete {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(deleteData("name", request.getParameter("name"), "food")) {
			response.sendRedirect("/bookBar/backStage/menu/foodList.jsp?delete=true&tag=" + URLEncoder.encode(request.getParameter("tag"), "UTF-8"));
		}
		else {
			response.sendRedirect("/bookBar/backStage/menu/foodList.jsp?delete=false&tag=" + URLEncoder.encode(request.getParameter("tag"), "UTF-8"));
		}
	}
